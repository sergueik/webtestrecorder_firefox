...
var wtr_WebtestRecorder = {
    ...
    toString: function() {
        return '[object wtr_WebtestRecorder]'
    },
    listeners_: [],
    ...
    addStep: function(_oStep, _oNode) {
        if (_oNode)
            this.checkFrame_(_oNode);
        this.addStep_(_oStep);
    },

    addStep_: function(_oStep) {
            ...
            if (this.curDeepFrameName_ != oWin.name) {
                ...
                try {
                    while (tmpWin.frameElement) {
                        this.addStep_(new WTRStep('followFrame', {
                            name: tmpWin.name
                        }));

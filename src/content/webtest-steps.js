...
wtr_Misc.log = new Log("Webtest-steps");
...
WTR_ButtonLocator.locate = function(_oTarget)
{
	...
	if (stepsVariations.length == 0) // better than nothing?
		stepsVariations.push(new WTRStep("clickButton"))
...
{
	if (!window.WTRStep) {
		return // seems that click handler is still registered when it should not be
	}
	var myThis = arguments.callee.myThis;
	...
	try
	{
		...
		switch (_oField.type)
		{
			...
			case "select-multiple":
				oBaseStep = new WTRStep("setSelectField");
